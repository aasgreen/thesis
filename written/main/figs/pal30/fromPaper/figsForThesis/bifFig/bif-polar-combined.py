...
with plt.style.context('prl'):
   ...
   #fig,(ax,ax2) = plt.subplots(nrows=2,ncols=1,s
    ...
    ax3.set_xlabel(r'temperature (\si{\degreeCelsius})')
    ax3.set_ylabel(r'd (\AA)')
    ax3.set_ylim([40,160])
...
#tempi = np.linspace(120,70,100)
    timei = np.linspace(t0,tf,200)-z100
    unitless = 10**(-3)*39.9
    volti = griddata( (dfsort['Temperature'].values,dfsort['ch1'].values-z100,), dfsort['ch2'].values, (tempi[None,:], timei[:,None]),method='cubic')
    levels = [-0.5,-.2,-.1,-.05,0,.05,.1,.25,.50,.556,0.8,1]
    ...
    axv = ax2.twinx()
    axv.set_ylim([t0-z100,tf-z100])
    axv.yaxis.set_major_locator(loct)
    axv.yaxis.set_minor_locator(locminort)
    
    #ax2.axvline(thid,alpha=.1,color='black')
    axv.yaxis.set_major_formatter(plt.FuncFormatter(lambda x,pos:r'{:3.1f}'.format(10/5.86*voft(x))))
    ...
    cbaxes = inset_axes(ax2, width="5%", height = "40%", loc=2)
    cbar=plt.colorbar(CSf,cax=cbaxes)
    cbar.ax.set_yticklabels([],fontsize=10)
    cbar.outline.set_edgecolor("white")
    cbar.solids.set_edgecolor("face")
    yt1 = plt.getp(cbar.ax.axes,'yticklabels')
