...
tGroups = sm2D1.groupby('Temperature')
...
ax4 = fig4.add_subplot()
tempi = np.linspace(*sm2Trange,100)
timei = np.linspace(5,43,200)
avolt = np.linspace(-15,20,200)
cData = sm2D1.sort_values(['Temperature','ch1'])
slope = ((np.sign(np.diff(cData['ch3']))+1)/2).astype(bool)
slope=np.append([False],slope)
volti = griddata( (cData['ch3'][cData['slope']].values, cData['Temperature'][cData['slope']].values), cData['ch2'][cData['slope']].values, (avolt[None,:], tempi[:,None]),method='linear')


X,Y = np.meshgrid(avolt,tempi)
levels = [-0.8,0,.01,.05,.1,.37,.50,.57,.9]
CS = plt.contour(X,Y,volti*gain*1000,30,linewidths=0.1,colors='k',levels=[l*100 for l in levels])
...
ax4.set_xlim([-10,10])
ax4.set_ylim([99,109])
cbbox = inset_axes(ax4, width="20%", height = "90%", loc=2)
...
def cc(temp):
    norm = mcolors.Normalize(vmin=96,vmax=112)
...
for g in tGroups:
    ...
    xs = t['ch3'][t['slope']].values*10/5.86
    xSort = np.argsort(xs)
    xs = xs[xSort]
    ys = t['ch2Normed'][t['slope']].values*gain*1000
    ys = np.abs(ys[xSort])
    ys[0],ys[-1]=0,0
...
ax3.set_zlim3d(0,(sm2D1['ch2Normed'].max())*gain*1000)

fig3.tight_layout()
fig3.savefig('3dSm2PRC.png',dpi=200,bbox_inches='tight')
fig.tight_layout()
fig.savefig('spacedSm2PRC.png',bbox_inches='tight',dpi=200)
#plt.close('all')
##going to plot in 3d
