...
with plt.style.context('prl'):
    ...
    #Going to generate four plots: prc vs tilt and cumsum(prc) vs tilt for each temp.
    #We've already done the exploratory analysis
    gain = 1./20.
    fig,ax = plt.subplots()
...
#
    ax.plot(pos108['voltage'],pos108['normC']*gain*1000,'.')
    ax2 = ax.twinx()
    ...
    ax.set_xlabel(u'applied field (V/\u03bcm)')
    ax.set_ylabel(u'polarization current (nA)',color='C0')
    ax2.set_ylabel(u'optical tilt (deg)',color='C2')
    fig.tight_layout()
    fig.savefig('T108-PRVvTilt.svg')
    fig,ax=plt.subplots()

    ax.plot(pos108['voltage'],np.cumsum(pos108['normC'])/np.sum(pos108['normC']),'.',color='C3')
    ax2 = ax.twinx()
    ax2.plot(tilt108['E'][1::][tilt108['pos'][1::]], tilt108['Theta mean'][1::][tilt108['pos'][1::]], '.',c='C2')
    for t1 in ax2.get_yticklabels():
        t1.set_color('C2')
    ...
    ax.set_xlabel(u'applied field (V/\u03bcm)')
    ax.set_ylabel(u'polarization (a.u.)',color='C3')
    ax2.set_ylabel(u'optical tilt (deg)',color='C2')
    fig.tight_layout()
    
    fig.savefig('T108-PvTilt.svg')
    fig,ax = plt.subplots()
    ...
    ax.plot(pos110['voltage'],pos110['normC']*gain*1000,'.')
    ax2 = ax.twinx()
    ax2.plot(tilt110['E'][1::][tilt110['pos'][1::]], tilt110['Thetamean'][1::][tilt110['pos'][1::]], '.',c='C2')
    for t1 in ax2.get_yticklabels():
        t1.set_color('C2')
    for t1 in ax.get_yticklabels():
        t1.set_color('C0')
    ...
    fig.savefig('T110-PRCvTilt.svg')
    fig,ax=plt.subplots()

    ax.plot(pos110['voltage'],np.cumsum(pos110['normC'])/np.sum(pos110['normC']),'.',color='C3')
    ax2 = ax.twinx()
    ax2.plot(tilt110['E'][1::][tilt110['pos'][1::]], tilt110['Thetamean'][1::][tilt110['pos'][1::]], '.',c='C2')
    for t1 in ax2.get_yticklabels():
        t1.set_color('C2')
