...
tGroups = smRD1.groupby('Temperature')
...
ax4 = fig4.add_subplot()
tempi = np.linspace(*smRRange,100)
...
levels = [0,15,30,45,60,75,90,105,120]
...
ax4.set_xlim([-10,10])
ax4.set_ylim(smRRange)
cbbox = inset_axes(ax4, width="25%", height = "90%", loc=2)
...
def cc(temp):
    norm = mcolors.Normalize(vmin=smRRange[0]-20,vmax=smRRange[1]+10)
...
ax3.set_zlim3d(0,(smRD1['ch2Normed'].max())*gain*1000)

fig3.tight_layout()
fig3.savefig('3dSmRPRC.png',dpi=200,bbox_inches='tight')
fig.tight_layout()
fig.savefig('spacedSmRPRC.png',bbox_inches='tight',dpi=200)
#plt.close('all')
##going to plot in 3d
#figMain.tight_layout()
#figMain.savefig('pal30-sm2-prc.pdf',bbox_inches='tight')
plt.close('all')
tGroups = smRData.groupby('Temperature')
fig,ax = plt.subplots(figsize=(4,10))
pltTemps = smRData['Temperature'].unique()
n = len(pltTemps)
...
fig.tight_layout()
fig.savefig('vVa-smR.png',bbox_inches='tight')
